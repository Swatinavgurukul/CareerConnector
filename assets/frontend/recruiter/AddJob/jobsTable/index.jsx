...
class ListPagination extends Component {
    ...
    getData() {
        let  dataToSort = []
        if(this.state.sortBy === "display_name"){
            let dataCopy = JSON.parse(JSON.stringify(this.props.data));
            dataToSort = dataCopy.map(list => {
                return   {
                    ...list,
                    sortKey: list.display_name ? list.display_name : "Remote Location"
                }
            })
         
        }else{
            dataToSort = JSON.parse(JSON.stringify(this.props.data));
        }
        let sortBy = this.state.sortBy;
        let order = this.state.ascending ? 1 : -1;
        let render_data = [];
        let sorted_data = dataToSort.sort(function (a, b) {
        ...
        return this.state.sortBy == key ? (
            this.state.ascending ? (
                <span className="btn-svg-sm icon-invert">
                    <img src="/svgs/icons_new/chevron-up.svg" alt="chevron-up" className="svg-xs invert-color" />
                </span>
            ) : (
                <span className="btn-svg-sm icon-invert">
                    <img src="/svgs/icons_new/chevron-down.svg" alt="chevron-down" className="svg-xs invert-color" />
                </span>
            )
        ) : (
            ""
    ...
    setSelect = (e) => {
        ...
        let renderData = JSON.parse(JSON.stringify(this.state.render_data));
    ...
    render() {
        ...
        return (
            <div className="">
                {/* <form className="text-right pt-3">
                    <input type="text" placeholder="Search" value={this.state.query} onChange={this.onSearch} />
                </form> */}
                <div className="d-flex justify-content-between mt-3 mb-1">
                    <small className="text-muted d-flex align-items-end">
                        {t(this.props.language?.layout?.js_showing)} {this.state.firstIndex}-{this.state.lastIndex}{" "}
                        {t(this.props.language?.layout?.js_of)} {this.state.filtered_data.length}{" "}
                        {t(this.props.language?.layout?.js_records)}
                    </small>
                    {/* <div className="d-flex align-items-center text-muted">
                        <img
                            className="svg-xs mr-1 svg-gray"
                            src="svgs/icons/settings-light.svg"
                            title="Settings"
                            alt="Settings Icon"
                        />{" "}
                        Columns
                                ...
                                <tr>
                                    {this.props.tableJSON.map((item) => {
                                        return (
                                            <th
                                                scope="col"
                                                className={`pl-3 align-middle font-weight-normal border-bottom-0 ${
                                                    item.displayValue === "Actions" ? "text-center" : ""
                                                }`}
                                                style={
                                                    item.displayValue === "Title" ? { width: "25%" } : { width: "auto" }
                                                }
                                                onClick={
                                                    item.type === "checkbox"
                                                        ? ""
                ...
                <div className="row mt-3">
                    ...
                    {this.state.itemsPerPage < this.state.filtered_data.length ? (
