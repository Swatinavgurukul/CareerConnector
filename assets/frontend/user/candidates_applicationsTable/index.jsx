...
class ListPagination extends Component {
    ...
    getData() {
        ...
        if(this.state.sortBy === "username"){
            ...
            dataToSort = dataCopy.map(list => {
                return   {
                    ...list,
                    sortKey: list.first_name ? capitalizeFirstLetter(list.first_name) : capitalizeFirstLetter(list.username)
                }
            })
        ...
        let sorted_data = dataToSort.sort(function (a, b) {
            if (sortBy === "weighted_score")
                return parseInt(a[sortBy]) > parseInt(b[sortBy]) ? order : -order;
    ...
    sortBy = (key) => {
        key === this.state.sortBy
            ? 
            this.setState({ ascending: !this.state.ascending, sortBy: key, activeIndex: 0 }, () => {
                this.getData();
            })
                                ...
                                <tr>
                                    {this.props.tableJSON.map((item) => {
                                        return (
                                            <th
                                                scope="col"
                                                className={`pl-3 align-middle font-weight-normal ${item.displayValue == "Actions" ? "text-center" : ""
                                                    }`}
                                                style={
